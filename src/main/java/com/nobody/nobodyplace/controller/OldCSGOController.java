...
public class OldCSGOController {
    ...
    public OldCSGOController(OldCsgoService service, CommonStorageService commonService) {
    ...
    public ResultPast getUserProperty() {
    ...
    public ResultPast batchGetItemHistoryPrice(@RequestBody List<RequestItemHistoryPrice> requestItems) {
        if (!hasValidBuffCookie()) {
            buffCookie = "";
            Nlog.info("batchGetItemHistoryPrice... require BUFF login");
            return new ResultPast(500, "Login required");
        }
        long delayTime = 1000;
    ...
    public ResultPast getUserTransaction(@RequestBody RequestGetUserTransaction request) {
        if (request.from > request.to) {
            return new ResultPast(-1, "Invalid request body");
        }
        // 一天自动更新一次，除非走强制更新
        if (System.currentTimeMillis() - leaseRecordUpdateTime > TimeUtil.MILLI_DAY || request.fetch == 1) {
            if (!getLeaseRecords()) {
                ...
                return new ResultPast(-1, "Can not get lease records. Check internet or cookie.");
            }
    ...
    public ResultPast getIncomeStatus(@RequestBody RequestGetIncomeStatus request) {
        ...
        return new ResultPast(-1, "unknown income type");
    }

    private ResultPast getLeaseIncomeStatus(RequestGetIncomeStatus request) {
        if (System.currentTimeMillis() - leaseRecordUpdateTime > TimeUtil.MILLI_DAY || request.fetch == 1) {
            if (!getLeaseRecords()) {
                return new ResultPast(-1);
            }
     ...
     * 获取持有的潜在收入
     */
    private ResultPast getHoldingIncomeStatus(RequestGetIncomeStatus request) {
        if (itemCache.isEmpty()) {
            fetchAllItemDetails();
        }
        float cost = 0F;
