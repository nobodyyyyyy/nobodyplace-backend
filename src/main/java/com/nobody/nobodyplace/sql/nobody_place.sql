...
CREATE TABLE `info_csgo_user_transaction` (
  `item_id` int NOT NULL,
  `transact_id` varchar(50) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  ...
  `transact_time` int DEFAULT NULL COMMENT '如果是租赁的，为归还',
  PRIMARY KEY (`transact_id`,`item_id`) USING BTREE,
  KEY `user_transaction_item_id` (`item_id`),
