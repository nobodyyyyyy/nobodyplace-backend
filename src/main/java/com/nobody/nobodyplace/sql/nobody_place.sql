...
CREATE TABLE `info_csgo_price_history` (
  ...
  PRIMARY KEY (`transact_time`,`item_id`),
  KEY `price_history_item_id` (`item_id`),
  CONSTRAINT `price_history_item_id` FOREIGN KEY (`item_id`) REFERENCES `info_csgo_item` (`item_id`)
) ENGINE=InnoDB DEFAULT CHARSET=ascii COLLATE=ascii_bin;
